...
export default function MusicManager({ onUploadSuccess }: MusicManagerProps) {
    ...
    const handleCoverImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            if (file.size > 5 * 1024 * 1024) {
                setUploadError('A imagem de capa é muito grande. Tamanho máximo: 5MB')
                setCoverImage(null)
                return
            }
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (selectedFile) {
            ...
            if (selectedFile.size > 50 * 1024 * 1024) {
                setUploadError('O arquivo de áudio é muito grande. Tamanho máximo: 50MB')
                setFile(null)
                return
            }
            setUploadError(null)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Validation
        if (!file) {
            setUploadError('Por favor, selecione um arquivo de áudio')
            return
        }

        if (!title.trim()) {
            setUploadError('Por favor, insira um título para a música')
            return
        }

        if (!artist.trim()) {
            setUploadError('Por favor, insira o nome do artista')
            return
        }
        ...
        <div className="modal-overlay" onClick={() => setIsOpen(false)}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="upload-form">
                    ...
                    <div className="form-group">
                        <label htmlFor="coverImage">Foto de Capa (Opcional - Máx: 5MB)</label>
                        <input
                            id="coverImage"
                            type="file"
                            accept="image/*"
                    ...
                    <div className="form-group">
                        <label htmlFor="file">Arquivo de Áudio (MP3, WAV, etc. - Máx: 50MB)</label>
                        <input
                            id="file"
                            type="file"
                            accept="audio/*"
